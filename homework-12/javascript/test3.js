...
if (confirm(`Почати   тестування?`)) {
    ...
    const names = ['John', 'Emily', 'Michael', 'Sarah', 'David', 'Olga', 'Jessica', 'Daniel', 'Sophia', 'Matthew'];
     ...
     * @returns - індекс елементу
     */
    function findIndex(arr, lengthElement) {
        let start = 0;
        let end = arr.length - 1;
        while (start <= end) {
            const middle = Math.floor((start + end) / 2);
     ...
     */
    const getRandomMatrix = (rowCount, elCount, min, max) => Array.from({ length: rowCount }, () => getRandomRow(elCount, min, max));
