if (confirm(`Почати   тестування?`)) {
	...
	// 1) Загальну вартість (нові ціни - price)  (let dataList - основний масив)

	let totalSum: number = dataList.reduce((prevSum, product) => prevSum + product.price, 0)

	document.write(`1.Загальна вартість  - ${totalSum}<br>`)

	// 2)Знайти кількість товарів, у яких ціна зменшилась (price < old_price).

	let productsPriceDecreased: number = dataList.reduce((prevCount, product) =>
		(product.price < product.old_price ? prevCount + 1 : prevCount), 0)

	document.write(`2.кількість товарів, у яких ціна зменшилась   - ${productsPriceDecreased}<br>`)

	// 3) Товари, які доступні (sell_status:"available")

	let productsAvailable: number = dataList.reduce((prevCount, product) =>
		(product.sell_status === 'available' ? prevCount + 1 : prevCount), 0)

	document.write(`3.кількість товарів, які доступні   - ${productsAvailable} - масив у консолі <br>`)

	const productsAvailableArr: List[] = dataList.filter((product) => product.sell_status === 'available')

	console.log(productsAvailableArr)
	...
	const productsAvailableArrNew: New[] = productsAvailableArr.map(({ id, price, old_price, usd_price }) =>
		({ id, price, old_price, usd_price }))
	console.log(productsAvailableArrNew)

	document.write('4.масив у консолі <br>')
